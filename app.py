...
rag_answer_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "Answer using the context. If the answer is not in the context, say: "
            "'I don't know based on the uploaded file.'\n\nContext:\n{context}",
        ),
        MessagesPlaceholder("chat_history"),
        ("human", "{input}"),
    ]
...
def get_llm(model_name, temperature, max_tokens):
    ...
    try:
        ...
        return ChatGroq(model=DEFAULT_MODEL, temperature=temperature, max_tokens=max_tokens, api_key=api_key)
...
with t1:
    st.markdown(
        '<div style="padding:6px 0 2px 0;"><span style="font-size:1em;font-weight:600;color:#ececec;">AI Assistant</span>'
        '<span style="font-size:0.7em;color:#555;margin-left:8px;">Groq · LangChain · RAG</span></div>',
        unsafe_allow_html=True,
    )
with t2:
    st.markdown(
        f'<div style="text-align:center;padding:6px 0;"><span style="font-size:0.75em;color:#555;">'
        f'You <b style="color:#aaa;">{user_msgs}</b> &middot; AI <b style="color:#aaa;">{ai_msgs}</b></span></div>',
        unsafe_allow_html=True,
    )
with t3:
...
if st.session_state.get("show_settings", False):
    ...
    with p1:
        st.caption("MODEL")
        model_name = st.selectbox("Model", MODEL_OPTIONS, label_visibility="collapsed", key="model_sel")
    with p2:
    ...
    model_name = st.session_state.get("model_sel", DEFAULT_MODEL)
...
if st.session_state.messages:
    for message in st.session_state.messages:
        if message["role"] == "user":
            st.markdown(
                f'<div class="user-message">{message["content"]}<div class="timestamp">{message["timestamp"]}</div></div>',
                unsafe_allow_html=True,
            )
        else:
            ...
            if message.get("sources"):
                ...
                st.markdown(
                    f'<div style="font-size:0.7em;color:#444;margin-top:2px;">Answered from {n} section{"s" if n > 1 else ""}</div>',
                    unsafe_allow_html=True,
                )
else:
    st.markdown(
...
if st.session_state.rag_ready and not st.session_state.get("show_settings"):
    st.markdown(
        f'<div class="file-badge"><span style="color:#4ade80;font-weight:600;">RAG</span>'
        f'<span style="color:#888;margin-left:8px;">{st.session_state.file_name}</span></div>',
        unsafe_allow_html=True,
    )
